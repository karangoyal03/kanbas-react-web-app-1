// src/Labs/Lab3/Classes.tsx

import React from 'react';
import './Classes.css'; // Import the CSS file for styling

export default function Classes() {
  const color = 'blue'; // Define a color variable for dynamic class application
  const dangerous = true; // Define a boolean variable for conditional class application

  return (
    <div id="wd-classes">
      <h2>Classes</h2>
      <div className="wd-bg-yellow wd-fg-black wd-padding-10px">
        Yellow background
      </div>
      <div className="wd-bg-blue wd-fg-black wd-padding-10px">
        Blue background
      </div>
      <div className="wd-bg-red wd-fg-black wd-padding-10px">
        Red background
      </div>
      <hr />
      <div className={`wd-bg-${color} wd-fg-black wd-padding-10px`}>
        Dynamic {color} background
      </div>
      <div className={`${dangerous ? 'wd-bg-red' : 'wd-bg-green'} wd-fg-black wd-padding-10px`}>
        {dangerous ? 'Dangerous background' : 'Safe background'}
      </div>
    </div>
  );
}
